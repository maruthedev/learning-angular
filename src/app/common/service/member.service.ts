...
export class MemberService {
  ...
  async login(user: User): Promise<Member | undefined> {
    ...
    try {
      this.loggedMember = new Member(response.id, response.name, response.gender ?? undefined, response.age, response.tel, response.role, response.is_active, response.avatar_image_url, response.email, null);
    } catch (exception: any) {
      console.error(exception);
    }
